...
max_step = 100
scenarios, *_ = parse_config()
dynamics = parse_dynamics()
suffix = 'tsp_static'


class StaticTSPSolverRNN:
    ...
    def __get_state(self):
        ...
        for i, task_node_idx in enumerate(self.task_nodes_idx):
            if self.visited[task_node_idx] == 1: continue
            state_ = np.array([task_node_idx, self.cur_node_idx, self.end_node_idx])
            state.append(state_)

        if len(state) <= 0: state = [np.zeros(3, )]
    ...
    def step(self, action):
        ...
        if done:
            ...
            reward -= haversine(nodes[next_node], nodes[end_node])
    ...
    def test(self, num_episodes=1000, name='fig'):
        ...
        for ep in range(num_episodes):
            ...
            print('Task nodes: ', [self.nodes[u] for u in self.task_nodes_idx])

            # Reinforcement Learning
            total_reward, episode_step = 0.0, 0
            path_rl = [self.start_node, ]
            while not done_rl:
            ...
            for i, u in enumerate(all_nodes):
                for j, v in enumerate(all_nodes):
                    if i == j: continue
                    dist_matrix[i][j] = haversine(nodes[u], nodes[v])
...
def main():
    ...
    eps_decay = int(num_episodes * 5 * 0.2)

    graph, *_ = load_graph(place_name)
    planner = StaticTSPSolverRNN(graph, epsilon_decay=eps_decay)
