...
from train_encoder import DQNEncoder
...
class DPPWithEncoder:
    def __init__(self, graph, p,
        ...
        self.q_network = DQNEmbedding(input_dim=state_dim,
                                      output_dim=num_action,
                                      other_dim=feature_size)
        self.target_network = DQNEmbedding(input_dim=state_dim,
                                           output_dim=num_action,
                                           other_dim=feature_size)
        ...
        self.encoder = DQNEncoder(input_dim=len(graph.edges), output_dim=feature_size)
        state_dict = torch.load('../trained/model_encoder.pth')
        self.encoder.load_state_dict(state_dict)
        self.has_dynamics = has_dynamics
        self.dyn_features = None

    def __get_state(self):
        node_arr = np.array([self.cur_node_idx, self.end_node_idx])
        return np.concatenate([node_arr, self.dyn_features], dtype=np.float32)
    ...
    def reset(self, reuse=False):
        if not reuse:
            ...
            if self.has_dynamics:
                ...
                _, edge_attr = add_dynamics(self.graph, dynamic)
                edge_attr_tensor = torch.tensor(edge_attr, dtype=torch.float32)
                dyn_feature_tensor = self.encoder(edge_attr_tensor)
                self.dyn_features = dyn_feature_tensor.squeeze(0).detach().numpy()
    ...
    def update_q_network(self, t):
        ...
        for key, batch in batch_dict.items():
            ...
            states_tensor = torch.tensor(np.array(states), dtype=torch.int32).squeeze(0)
            ...
            next_states_tensor = torch.tensor(np.array(next_states), dtype=torch.int32).squeeze(0)
    ...
    def test(self, num_episodes=1000):
        ...
        for ep in range(num_episodes):
            ...
            for i, node1 in enumerate(path_di[:-1]):
                node2 = path_di[i + 1]
                cost = self.graph[node1][node2][0]['dynamic_weight']
                cost_di += cost
                print('\t', i,
                      self.nodes.index(node1),
                      self.nodes.index(node2), cost)
            print("\t Optimal path (DI):", round(cost_di, 2), done_di, path_di)

            stats.append([self.start_node, self.end_node, int(done_rl), cost_rl, int(done_di), cost_di])

        with open('trained/result_{}.csv'.format(suffix), 'w', newline='') as f:
...
def main():
    ...
    eps_decay = int(num_episodes * max_step * 0.2)

    graph, p, num_action = load_graph(place_name)
    planner = DPPWithEncoder(graph,
                             p, num_action,
