...
num_paths = int(1e4)
k = 1.0
atte = load_pa_table(num_paths=num_paths)
edge_dict = {key_: value[str(k)]['edges'] for key_, value in atte.items()}
other_dim_ = max(len(value) for value in edge_dict.values())
print('k (PA): ', k, other_dim_)
suffix = 'dpp_pa_{}'.format(k)
...
class DPPDynamicPA:
    def __init__(self, graph, p,
        ...
        self.q_network = DQNEmbedding(input_dim=state_dim, output_dim=num_action, other_dim=other_dim_)
        self.target_network = DQNEmbedding(input_dim=state_dim, output_dim=num_action, other_dim=other_dim_)
    ...
    def reset(self, reuse=False):
        if not reuse:
            ...
            key = '{}-{}'.format(self.start_node_idx, self.end_node_idx)
            _, edge_attr = add_dynamics(self.graph, dynamic, limitations=edge_dict[key])
            if len(edge_attr) < other_dim_:
                fill_arr = np.zeros((other_dim_ - len(edge_attr), ))
                self.dyn_features = np.concatenate([edge_attr, fill_arr])
            else:
                self.dyn_features = np.array(edge_attr)
    ...
    def train(self, num_episodes=1000, **kwargs):
        ...
        for episode in range(1, num_episodes + 1):
            ...
            if episode % 100 == 0:
                ...
                if episode % 5000 == 0:
                    self.test(num_episodes=100, epoch=episode)
